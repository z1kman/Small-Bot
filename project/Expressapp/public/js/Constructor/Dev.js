...
function PublishProject(id) {//публикация проекта
    let Code = "";
    ...
    clearTimeout(timerId);//очиста таймера, чтобы сообщения об ошибке или о сохранении проекта не висели
    Code = GenerateCode(id);//генерирование кода
...
function SaveProject() {//сохранение проекта
    ...
    fetch(urlConstr, { //post запрос на сервер со всеми данными
        method: 'POST',
        headers: {
...
function TestProject(id) { //тестирование проекта
    let Code = "";
    if (document.getElementById("DivErrorProject") != undefined) {//если панель с ошибкой присутствует на форме, тогда удалить ее
        document.getElementById("DivErrorProject").remove();
    }
...
function GenerateCode(ButtonName) {//генерирование кода
    ...
    for (let i = 0; i < Canvas.length; i++) {//поиск наличия начальной панели
        N = GetNumberOfElement(Canvas[i].id);//получение первого числа из id элемента
        SN = GetSecondNumberOfElement(Canvas[i].id);//получение второго числа из id элемента
        TN = GetThirdNumberOfElement(Canvas[i].id);//получение треьего числа из id элемента
        StartPanelId = "Panel " + N + " " + SN;//создание id стартовой панели
        if (document.getElementById(StartPanelId).hasAttribute('data-connect-0') == false)//Проверка на наличие связей у панели
        {
            break;//если связь присутствует, то осущетвляется выход из цикла
    ...
    for (let i = 0; i < Variables.length; i++) {//генерирование переменных и проверка на синтаксическую правильность
        Code += "\n var " + Variables[i].innerHTML.replace(/\s+/g, '') + "= \"\" ;";//запись имени переменных в код будущего чат-бота
        let Variable = Variables[i].innerHTML.replace(/\s+/g, '');//получение текущего имени переменной
        for (let e = 0; e < Variable.length; e++) {//цикл по всем переменым для проверки правильности имени переменной 
            if ((Variable[0] > 'A' && Variable[0] < 'Z') || (Variable[0] > 'a' && Variable[0] < 'z')) {//если первый элемент в имени переменной содержит латинские буквы
                continue;//переход к следующей итерации
            }
            //Если текущая итерация не для первого элемента и перый элемент это только латинские буквы а все остальные это латинские буквы и цифры
    ...
    Code += "\n var DialogueStarted = 0;" + //флаг для проверки ответа пользователя на действия чат-бота
        "\n var MessageBeforeDialogue = new Array();" + //переменная для хранения сообщений чат-бота которые не были отправлены на сервер(сообщения до ответа пользователя)
        "\n var Dialog = + Math.floor(Math.random() * (9999999999 - 1000000000)) + 1000000000;";//генерирование случайного номера диалога
    ...
    for (let i = 0; i < Panels.length; i++) {//Цикл по всем панелям
        for (let j = 0; j < Panels[i].childNodes.length; j++) {//Цикл по всем дочерним элементам панелей
            if (document.getElementById(Panels[i].childNodes[j].id).className == 'Bot') {//если обнаружен блок действий бота;
                for (let k = 0; k < Panels[i].childNodes[j].childNodes.length; k++) {//Цикл по всем элементам блока действий чат бота
                    if (Panels[i].childNodes[j].childNodes[k] != undefined)//проверка на не пустой элемент
                    {
                        ...
                        if (Panels[i].childNodes[j].childNodes[k].className == "TextBot" || Panels[i].childNodes[j].childNodes[k].className == "ImgBot")//если найден текстовый элемент или изображение
                        {
                            ...
                            N = GetNumberOfElement(ElementId);//получение первого числа id элемента
                            SN = GetSecondNumberOfElement(ElementId);//получение второго числа id элемента
                            TN = GetThirdNumberOfElement(ElementId);//получение третьего числа id элемента
                            let Text = "";
                            let Img;
                            ...
                            if (GetNumberOfElement(StartPanelId) == N && GetSecondNumberOfElement(StartPanelId) == SN && StartExist == false) //если текущая панель - начальная панель
                            {
                            ...
                            if (Panels[i].childNodes[j].childNodes[k + 1].className == "TextBot" || Panels[i].childNodes[j].childNodes[k + 1].className == "ImgBot") {//если существует еще одно действие вывода текста у бота или изображения на текущей панели
                                ...
                                Code += "\n\t Act_" + GetNumberOfElement(NextStepId) + "_" + GetSecondNumberOfElement(NextStepId) + "_" + GetThirdNumberOfElement(NextStepId) + "();";
                                    ...
                                    if (User.childNodes[f] != undefined) {//проверка на не пустой элемент
                                        if (User.childNodes[f].className == "DivUserElement") {//если следующий элемент - текст/число/емеил
                                            ...
                                            Code += "\n\t let SendMessage = document.getElementById('SendMessage')" +
                                                "\n\t SendMessage.setAttribute('onclick','Act_" + GetNumberOfElement(NextStepId) + "_" + GetSecondNumberOfElement(NextStepId) + "_" + GetThirdNumberOfElement(NextStepId) + "(); InputMessage.value = \"\" ; ')";
                                            break;
                                            ...
                                            for (let e = 0; e < Panels[i].childNodes.length; e++) {//запуск цикла по всем элементам текущей панели
                                                if (Panels[i].childNodes[e].className == "DivJumpIndicator") {//поиск джампера находящегося на панели
                                                    let JumpIndicator = document.getElementById("JumpIndicator " + GetNumberOfElement(Panels[i].childNodes[e].id) + " " +
                                                        GetSecondNumberOfElement(Panels[i].childNodes[e].id) + " " + GetThirdNumberOfElement(Panels[i].childNodes[e].id));
                ...
                for (let k = 0; k < Panels[i].childNodes[j].childNodes.length; k++) {//Цикл по всем элементам блоков действий пользователя
                    if (Panels[i].childNodes[j].childNodes[k] != undefined) {//проверка на не пустой элемент
                        ...
                        if (Panels[i].childNodes[j].childNodes[k].className == "DivUserButton") {//Если текущий элемент - кнопка
                            ...
                            if (GetNumberOfElement(StartPanelId) == N && GetSecondNumberOfElement(StartPanelId) == SN && StartExist == false) //если это начальная панель
                            {
                                StartExist = true;//установления флага в значение - функция старта существует
                                Code += "\nwindow.onload = function(){" +
                                    "\n\t Start(); \n}" +
                            ...
                            if (JumpIndicator.classList.contains('ActiveJumpIndicator')) {//Если есть соединение с другой панелью
                                ...
                                let Bot = document.getElementById("Bot " + GetNumberOfElement(PanelConnect.id) + " " + GetSecondNumberOfElement(PanelConnect.id));
                                let User = document.getElementById("User " + GetNumberOfElement(PanelConnect.id) + " " + GetSecondNumberOfElement(PanelConnect.id));
                                let Condition = document.getElementById("Condition " + GetNumberOfElement(PanelConnect.id) + " " + GetSecondNumberOfElement(PanelConnect.id));
                                    ...
                                    for (let f = 0; f < Bot.childNodes.length; f++) {//поиск элементов бота у присоединеной панели
                                        if (Bot.childNodes[f] != undefined) {//проверка на не пустой элемент
                                            if (Bot.childNodes[f].className == "TextBot") {//если обнаружен текстовый элемент
                                                let TextBotId = GetNumberOfElement(Bot.childNodes[f].id) + "_" + GetSecondNumberOfElement(Bot.childNodes[f].id) + "_" + GetThirdNumberOfElement(Bot.childNodes[f].id);
                                        ...
                                        for (let f = 0; f < User.childNodes.length; f++) {//поиск кнопок в действиях пользователя в присоединенной панели
                                            if (User.childNodes[f] != undefined) {//проверка на не пустой элемент
                                                if (User.childNodes[f].className == "DivUserButton") {//если обнаружена кнопка
                                                    let ButtonId = GetNumberOfElement(User.childNodes[f].id) + "_" + GetSecondNumberOfElement(User.childNodes[f].id) + "_" + GetThirdNumberOfElement(User.childNodes[f].id);
                                                    //запись в код вызов функции. при нажатии по кнопке вызовется действие бота
                                        ...
                                        for (let f = 0; f < User.childNodes.length; f++) {//поиск других действий пользователя в присоединеной панели
                                            if (User.childNodes[f] != undefined) {//проверка на не пустой элемент
                                                if (User.childNodes[f].className == "DivUserElement") {//если обнаружен какой либо элемент действия пользователя
                                                    let ID = GetNumberOfElement(User.childNodes[f].id) + "_" + GetSecondNumberOfElement(User.childNodes[f].id) + "_" + GetThirdNumberOfElement(User.childNodes[f].id);
                                                    //запись в код вызов функции. при нажатии по кнопке вызовется действие бота
                                    ...
                                    for (let f = 0; f < Condition.childNodes.length; f++) {//поиск элементов условий у присоединеной панели
                                        if (Condition.childNodes[f] != undefined) {//проверка на не пустой элемент
                                            if (Condition.childNodes[f].className == "DivConditionElement") {//если обнаружено условие
                                                let ConditionId = GetNumberOfElement(Condition.childNodes[f].id) + "_" + GetSecondNumberOfElement(Condition.childNodes[f].id) + "_" + GetThirdNumberOfElement(Condition.childNodes[f].id);
                        ...
                        //------------------------------Генерация функций для действий пользователя--------------------------------------------
                            ...
                            N = GetNumberOfElement(ElementId);
                            SN = GetSecondNumberOfElement(ElementId);
                            TN = GetThirdNumberOfElement(ElementId);
                ...
                let JumpIndicator = document.getElementById("JumpIndicator " + GetNumberOfElement(Panels[i].childNodes[j].id) + " " +
                    GetSecondNumberOfElement(Panels[i].childNodes[j].id) + " " + GetThirdNumberOfElement(Panels[i].childNodes[j].id));
                if (JumpIndicator.classList.contains('ActiveJumpIndicator')) {//если джампер активен
                    ElementId = GetNumberOfElement(JumpIndicator.id) + " " + GetSecondNumberOfElement(JumpIndicator.id) + " " + GetThirdNumberOfElement(JumpIndicator.id);
                    Code += "\n function Act_" + GetNumberOfElement(JumpIndicator.id) + "_" + GetSecondNumberOfElement(JumpIndicator.id) + "_" + GetThirdNumberOfElement(JumpIndicator.id) + "(){"
                    if (document.getElementById("Condition " + GetNumberOfElement(Panels[i].childNodes[j].id) + " " + GetSecondNumberOfElement(Panels[i].childNodes[j].id))) {//если текущая панель с условием
            ...
            for (let e = 0; e < Variables.length; e++) {//цикл по переменныи которые находятся в памяти (на проверку наличия переменных)
                if (Variables[e].innerHTML.replace(/\s/g, '') == Variable) {
                    ...
                    return ("error Ошибка! Переменная:\"" + Variable + "\", указанная на панели с именем:\"" +
                        document.getElementById("NamePanel " + GetNumberOfElement(PanelID) + " " + GetSecondNumberOfElement(PanelID)).innerHTML + "\", не найдена.");
                }
            }
