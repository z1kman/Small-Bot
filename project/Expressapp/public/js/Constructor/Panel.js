...
function OnClickRemovePanel(id) {//удаление панели
    ...
    for (let i = 0; i < CanvasesNumber; i++) {
        if (GetSecondNumberOfElement(arrCanvas[i].getAttribute('id')) == GetSecondNumberOfElement(id)) {
            let RemoveConnect = document.getElementById("RemoveConnect " + GetNumberOfElement(arrCanvas[i].getAttribute('id')) + " " + GetSecondNumberOfElement(arrCanvas[i].getAttribute('id')) + " " + GetThirdNumberOfElement(arrCanvas[i].getAttribute('id')));
    ...
    while (Connect == true) {//Поиск подключений
        if (Panel.hasAttribute('data-connect-0') == true) {//проверка на подключение к другой панели или элементу
            ...
            let RemoveConnect = document.getElementById("RemoveConnect " + GetNumberOfElement(IdConnect) + " " + GetSecondNumberOfElement(IdConnect) + " " + GetThirdNumberOfElement(IdConnect));
            OnClickRemoveConnect(RemoveConnect.getAttribute('id'));
            Connect == true;
            Null == true;
...
function OnClickNewConditionPanel(id) {//Создание новой панели условий
    ...
    if (GetNameOfElement(ParentIdNewPanel) == "ParentNewPanelBtn" && !ParentNewPanelBtn.classList.contains("active")) { //добавление новой секции
...
function OnClickJumpIndicator(id) {//нажатие на JumpIndicator
    ...
    if (JumpIndicator.classList.contains('ActiveJumpIndicator')) {
        let RemoveConnect = document.getElementById("RemoveConnect " + GetNumberOfElement(id) + " " + GetSecondNumberOfElement(id) + " " + GetThirdNumberOfElement(id));
...
function OnMouseOverDivJump(id) {//Панель. Мышь в области джампера
    let JumpIndicator = document.getElementById("JumpIndicator " + GetNumberOfElement(id) + " " + GetSecondNumberOfElement(id) + " " + GetThirdNumberOfElement(id));
    if (JumpIndicator.classList.contains('ActiveJumpIndicator')) {
        let RemoveConnect = document.getElementById("RemoveConnect " + GetNumberOfElement(id) + " " + GetSecondNumberOfElement(id) + " " + GetThirdNumberOfElement(id));
        if (!JumpIndicator.classList.contains('Active')) {
            let Canvas = document.getElementById("Canvas " + GetNumberOfElement(id) + " " + GetSecondNumberOfElement(id) + " " + GetThirdNumberOfElement(id));
...
function Jump(id) {//Стрелки.Создание стрелок между элементом и панелями
    if (flagA == true) {
        ...
        let N = GetNumberOfElement(IdOfParentJump);
        let SN = GetSecondNumberOfElement(IdOfParentJump);
        let TN = GetThirdNumberOfElement(IdOfParentJump);
        ...
        if (click == 1) {//если нажали первый раз
            ...
            for (let i = 0; i < Panels.length; i++) {//поиск панелей с такими же координатами что и были заданы
                if ((Panels[i].offsetLeft <= mouse.x && (Panels[i].offsetLeft + Panels[i].offsetWidth >= mouse.x)) &&
                    ...
                    if (GetSecondNumberOfElement(Panels[i].getAttribute("id")) != SN) {//чтобы не конектилась сама к себе
            ...
            if (Panel.offsetLeft > JumpIndicator.offsetLeft && Panel.offsetTop + 20 > JumpIndicator.offsetTop) { //создание канвы и ее позиционирование (4ая четверть)
            ...
            } else if (Panel.offsetLeft < JumpIndicator.offsetLeft && Panel.offsetTop + 20 > JumpIndicator.offsetTop) {//3я четверть
            ...
            } else if (Panel.offsetLeft > JumpIndicator.offsetLeft + 60 && Panel.offsetTop + 20 < JumpIndicator.offsetTop + 20) {//2ая четверть
                canvas.setAttribute("width", Panel.offsetLeft - 60 - JumpIndicator.offsetLeft);
            ...
            } else if (Panel.offsetLeft < JumpIndicator.offsetLeft && Panel.offsetTop + 20 < JumpIndicator.offsetTop) {//1ая четверть
            ...
            ctx.beginPath();
            if (Panel.offsetLeft > JumpIndicator.offsetLeft && Panel.offsetTop + 20 > JumpIndicator.offsetTop) {//рисование самого отрезка
                ctx.moveTo(0, 0);
...
function RefreshArrows() {//Стрелки.Перерисовка стрелок
    ...
    for (let i = 0; i < Arrows.length; i++) {
        ...
        let JumpIndicator = document.getElementById("JumpIndicator " + GetNumberOfElement(id) + " " + GetSecondNumberOfElement(id) + " " + GetThirdNumberOfElement(id));
        let Panel = document.getElementById("Panel " + GetNumberOfElement(idChildren) + " " + GetSecondNumberOfElement(idChildren));
